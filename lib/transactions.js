...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    if (rpcData.awards) {
        for (var i = 0; i < rpcData.awards.length; i++){
            var script = new Buffer(rpcData.awards[i].scriptPubKey.hex, 'hex');

            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(rpcData.awards[i].value),
